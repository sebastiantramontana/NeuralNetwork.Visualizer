...
namespace NeuralNetwork.Visualizer.Drawing.Canvas
{
   internal class GraphicsCanvas : ICanvas
   {
      ...
      public void DrawRectangle(Rectangle rect, Pen pen, IBrush brush)
      {
         ...
         using (var gdiBrush = brush.CreateBrush(rect))
         {

            if (Validate(gdiBrush))
               _Graph.FillRectangle(gdiBrush, rect);
         }
      ...
      public void DrawEllipse(Rectangle rect, Pen pen, IBrush brush)
      {
         ...
         using (var gdiBrush = brush.CreateBrush(rect))
         {
            if (Validate(gdiBrush))
            {
               _Graph.FillEllipse(gdiBrush, rect);
            }
         }
      }
      ...
      {
         var size = _Graph.MeasureString(text, font);

         using (var gdiBrush = brush.CreateBrush(new Rectangle(position, size.ToSize())))
         {
            if (Validate(gdiBrush))
               _Graph.DrawString(text, font, gdiBrush, position, format); //if the other args are null throw the exception
         }
      }

      public void DrawText(string text, FontInfo fontInfo, Rectangle rect, IBrush brush, StringFormat format)
      {
         using (var gdiBrush = brush.CreateBrush(rect))
         {
            if (Validate(gdiBrush))
               DrawAdjustedFontString(text, fontInfo, rect.Size, (font) => _Graph.DrawString(text, font, gdiBrush, rect, format));
         }
      }

      public void DrawText(string text, FontInfo fontInfo, Rectangle rect, IBrush brush, StringFormat format, float angle)
      {
         using (var gdiBrush = brush.CreateBrush(rect))
         {
            if (!Validate(gdiBrush))
            ...
            DrawAdjustedFontString(text, fontInfo, rect.Size, (font) =>
            {
               ...
               _Graph.DrawString(text, font, gdiBrush, new Rectangle(0, 0, rect.Width, rect.Height), format);
      ...
      {
         return !(brush is SolidBrush solidBrush && solidBrush.Color.A == 0);
      }

      private bool Validate(Pen pen)
      {
         return pen != null && pen.Color.A != 0;
      }
   }
}
